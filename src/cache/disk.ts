...
import { CacheEntry, PageMetadata } from "../types.js";
...
export class DiskCache {
  ...
  async init(): Promise<void> {
    try {
      ...
      if (process.env.DEBUG || process.env.CRAWL_DEBUG) {
        console.warn(
          `Cache directory creation failed: ${error instanceof Error ? error.message : "Unknown error"}. Caching will be disabled.`,
        );
      }
  ...
  async put(
    ...
    try {
      ...
      if (process.env.DEBUG || process.env.CRAWL_DEBUG) {
        console.warn(
          `Failed to write to cache: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
  }
