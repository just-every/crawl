...
import { CrawlOptions, CrawlResult, PageMetadata } from "../types.js";
...
import { extractMetadata } from "../parser/meta.js";
...
export class CrawlQueue {
  ...
  private options: Required<
    Omit<CrawlOptions, "cookieHeader" | "cookiesFile">
  > &
    Pick<CrawlOptions, "cookieHeader" | "cookiesFile">;
  ...
  private async processUrl(url: string, depth: number): Promise<void> {
    ...
    try {
      ...
      const cookieHeader =
        ...
        (this.cookieJar
          ? buildCookieHeaderForUrl(normalizedUrl, this.cookieJar)
          : undefined);
      ...
      // Cache the result
      await this.cache.put(normalizedUrl, markdown, article.title, metadata);
